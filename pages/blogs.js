    ...
    <div>
      ...
      <div className="bg-grey-50 my-12" id="blog">
        <div className="container mx-auto py-16 md:py-20">
          <h2 className="pt-6 text-center font-header text-4xl font-semibold text-[#5543af] (59 130 246 / 0.5) uppercase sm:text-5xl lg:text-6xl">
            ...
            {blogs.map((item) => {
              return <Link key={item.slug.current} href={"/blog/" + item.slug.current} className="shadow">
                <div><div style={{ "backgroundImage": `url(${builder.image(item.blogimage).width(200).url() || '/assets/img/post-01.png'})` }}
                  ...
                  <span
                    className="absolute right-0 bottom-0 mr-4 mb-4 block rounded-full bg-[#5543af]  px-6 py-2 text-center font-body text-sm font-bold uppercase text-purple-200 hover:bg-gray-500 md:text-base cursor-pointer">Read
                    More</span>
                </div>
                  <div className=" py-6 px-5 xl:py-8 cursor-pointer">
                    <span className="block font-body text-lg font-semibold text-[#5543af]"> {item.title}</span>
